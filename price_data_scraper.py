...
def get_price_data(stock, start_date):
    ...
    stock_prices = stock_prices["4. close"].to_frame('price') # save closing prices as series over dates as index
    ...
    s_p_prices = s_p_prices["4. close"].to_frame('s&p500') # save closing prices as series over dates as index
    s_p_prices = s_p_prices.loc[start_date:]
    
    return pd.concat([stock_prices,s_p_prices], axis=1, sort=False)